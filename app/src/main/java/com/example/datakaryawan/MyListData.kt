...
class MyListData : AppCompatActivity(), RecyclerViewAdapter.data_listener {
    ...
    private fun GetData() {
        ...
        getReference.child("Admin").child(getUserID).child("Data Karyawan")
    ...
    override fun OnDeleteData(data: Data_Karyawan?, position: Int) {
        val getUserID: String = auth?.currentUser?.uid.toString()
        val getrefrence = database.reference

        getrefrence.child("Admin").child(getUserID).child("Data Karyawan").child(data?.key.toString()).removeValue()
